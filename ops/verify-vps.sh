...
db_bindings="$(docker inspect -f '{{json .HostConfig.PortBindings}}' "$("${compose[@]}" ps -q db)")"
[[ "$db_bindings" == "{}" ]] || fail "DB port is published"
reg_bindings="$(docker inspect -f '{{json .HostConfig.PortBindings}}' "$registration_id")"
[[ "$reg_bindings" == "{}" ]] || fail "registration port is published"
